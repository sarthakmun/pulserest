...
import { convertPostmanAuthToPulseRESTAuth, scriptConversion } from '@/helpers'
import constants from '@/constants'

export async function convertPostmanExportToPulseRESTCollection(json: any, isZip: boolean, workspaceId: string) {
    if(isZip) {
        const zip = new JSZip()
...
function handlePostmanV2CollectionItem(postmanCollectionItem: any, parentId: string | null = null, workspaceId: string) {
    ...
    postmanCollectionItem.item.forEach((request: any) => {
        ...
        const authentication: RequestAuthentication = convertPostmanAuthToPulseRESTAuth(request.request)
        ...
        if(request.event) {
            ...
            plugins.push({
                ...
                'name': null,
                'code': {
                    'pre_request': scriptConversion(preScript, 'postmanToPulseREST'),
                    'post_request': scriptConversion(postScript, 'postmanToPulseREST')
                },
...
function importPostmanV2(collections: any[], workspaceId: string) {
    ...
    collections.forEach(postmanCollectionItem => {
        ...
        collection.push({
            ...
            children: convertedRequests,
            parentId: null,
            authentication: convertPostmanAuthToPulseRESTAuth(postmanCollectionItem),
            workspaceId
        })
    })
