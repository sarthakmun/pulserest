...
import { convertPostmanExportToPulseRESTCollection } from './postman'
...
test('importPostmanV2.0', async() => {
    ...
    const outputFile = await readFile(path.join(testDataFolder, 'PulseREST_2024-09-06.json'), 'utf-8')
    const expected = JSON.parse(outputFile)

    const converted: any = await convertPostmanExportToPulseRESTCollection(input, false, expected.collection[0].workspaceId)
...
test('importPostmanV2.1', async() => {
    ...
    const input = JSON.parse(inputFile)
    const outputFile = await readFile(path.join(testDataFolder, 'PulseREST_2024-09-06.v2.1.json'), 'utf-8')
    const expected = JSON.parse(outputFile)
