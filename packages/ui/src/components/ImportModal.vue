...
import {
    fileToJSON,
    fileToString,
    convertInsomniaExportToPulseRESTCollection,
    convertPulseRESTExportToPulseRESTCollection,
    convertOpenAPIExportToPulseRESTCollection,
    generateNewIdsForTree,
    convertCurlCommandToPulseRESTCollection,
    fetchWrapper,
    getSavedRequestTimeout
} from '@/helpers'
import { convertPostmanExportToPulseRESTCollection } from '@/parsers/postman'
        ...
        async importFile() {
            ...
            try {
                ...
                if(this.importFrom === 'Postman URL') {
                    ...
                    const { collection, plugins: newPlugins } = await convertPostmanExportToPulseRESTCollection(json, false, this.activeWorkspace._id)
                    ...
                    collectionTree = await convertOpenAPIExportToPulseRESTCollection(json, this.activeWorkspace._id)
                } else {
                    for(const fileToImport of this.filesToImport) {
                        ...
                        if(this.importFrom === 'Postman') {
                            const { collection, plugins: newPlugins } = await convertPostmanExportToPulseRESTCollection(json, fileToImport.name.endsWith('.zip'), this.activeWorkspace._id)
                            ...
                            collectionTree = collectionTree.concat(convertInsomniaExportToPulseRESTCollection(json, this.activeWorkspace._id))

                        } else if(this.importFrom === 'PulseREST') {
                            const { newCollectionTree, newPlugins } = convertPulseRESTExportToPulseRESTCollection(json, this.activeWorkspace._id)
                            ...
                            collectionTree = collectionTree.concat(await convertOpenAPIExportToPulseRESTCollection(exportAsString, this.activeWorkspace._id))
                        }
                    }
                }
        ...
        async importFromCurl(command) {
            try {
                if(this.startsWithCurl) {
                    this.importing = true
                    const result = await convertCurlCommandToPulseRESTCollection(command, this.activeWorkspace._id)
