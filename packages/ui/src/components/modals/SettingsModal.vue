                ...
                <div v-show="activeTab === 'request-response'" class="tab-panel">
                    <div>
                        ...
                        <div style="margin-top: 1rem">
                            Note that the default user agent <strong>PulseREST/{{ getVersion() }}</strong> is used when no global user agent is set here or on request level.
                        </div>
                    </div>
                    ...
                    <template v-if="flags.isElectron">
                        <div style="padding-top: 1rem">
                            ...
                            <div style="margin-left: 1.3rem; margin-top: 0.3rem;">Tick this if you're not able to make any requests despite the server being reachable. This is not recommended for most people and is only provided to temporarily alleviate issues with firewalls and vpns. See <a href="https://github.com/flawiddsouza/PulseREST/issues/86" target="_blank">this link</a> for more info.</div>
                        </div>
                    </template>
