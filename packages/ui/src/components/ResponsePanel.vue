        ...
        <div class="response-panel-tabs-context">
                ...
                <template v-if="shouldShowLargeResponseWarning">
                    <div class="content-box" style="text-align: center; padding: 2rem;">
                        ...
                        <p v-if="isBinaryResponse">
                            This response has media type <code style="background: var(--sidebar-item-active-color); padding: 2px 4px; border-radius: 3px;">{{ responseContentType }}</code> which is not recognized by PulseREST and may not display correctly in the preview.
                        </p>
                        ...
                        <div style="margin-top: 1.5rem; word-break: break-word;" v-if="response.error === 'Error: Request failed'">
                            ...
                            <template v-if="flags.isBrowser">
                                ...
                                <div v-show="!flags.browserExtensionEnabled" style="margin-top: 1.5rem; line-height: 1rem;">
                                    Points 4 & 5 can be bypassed using the <a href="https://chrome.google.com/webstore/detail/pulserest-cors-helper/pgoncladmcclnmilkbnmmbldcihdgfnf" target="_blank">Chrome</a> or <a href="https://addons.mozilla.org/en-US/firefox/addon/pulserest-cors-helper/" target="_blank">Firefox</a> extension for PulseREST
                                </div>
                                <div v-show="flags.browserExtensionEnabled" style="margin-top: 1.5rem; line-height: 1rem;">
            ...
            <div class="content-box" v-if="activeResponsePanelTab === 'Tests'">
                <div v-if="response.testResults.length === 0">
                    ...
                    <div>Please refer <a href="https://docs.pulserest.dev/plugins/testing-response-data.html" target="_blank">https://docs.pulserest.dev/plugins/testing-response-data.html</a> for more info on how to write tests.</div>
                </div>
                <div v-else>
    ...
    methods: {
        ...
        async copyResponseToClipboard() {
            if(!window.isSecureContext) {
                this.$toast.error('Copy to clipboard needs PulseREST running under a https url')
                return
            }
