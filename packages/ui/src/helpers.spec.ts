...
import {
    ...
    convertPostmanAuthToPulseRESTAuth,
    scriptConversion,
    toTree,
    getSpaces,
...
describe('Function: getSavedRequestTimeout', () => {
    test('returns saved positive timeout as an integer', () => {
        localStorage.setItem('PulseREST-RequestTimeout', '1500.9')
    ...
    test('returns 0 for missing or invalid timeout values', () => {
        ...
        localStorage.setItem('PulseREST-RequestTimeout', '-1')
        expect(getSavedRequestTimeout()).toBe(0)

        localStorage.setItem('PulseREST-RequestTimeout', 'abc')
        expect(getSavedRequestTimeout()).toBe(0)
    })
})
...
describe(`Function: ${fetchWrapper.name}`, () => {
    ...
    test('does not send extension request when file serialization finishes after timeout', async() => {
        vi.useFakeTimers()
        ;(window as WindowWithExtensionHook).__EXTENSION_HOOK__ = 'PulseREST CORS Helper Enabled'
...
describe('convertPostmanAuthToPulseRESTAuth', () => {
    test('should return No Auth when auth is not present', () => {
        const request = {}
        const result = convertPostmanAuthToPulseRESTAuth(request)
...
describe('scriptConversion', () => {
    test('should convert Postman script to PulseREST script with basic mappings', () => {
        const postmanScript = `
    ...
    `
        ...
        const result = scriptConversion(postmanScript, 'postmanToPulseREST')
        expect(result).toBe(expectedPulseRESTScript)
    })

    test('should convert PulseREST script to Postman script with basic mappings', () => {
        const pulserestScript = `
      rf.setEnvVar("key", "value");
      rf.getEnvVar("key");
      rf.response.getBodyJSON();
    `
    ...
    `
        ...
        const result = scriptConversion(pulserestScript, 'pulserestToPostman')
        expect(result).toBe(expectedPostmanScript)
    })

    test('should convert PulseREST script to Insomnia script with basic mappings', () => {
        const pulserestScript = `
      rf.setEnvVar("key", "value");
      rf.getEnvVar("key");
      rf.response.getBodyJSON();
    ...
    `
        ...
        const result = scriptConversion(pulserestScript, 'pulserestToInsomnia')
        expect(result).toBe(expectedInsomniaScript)
    })

    test('should convert Postman status code assertions to PulseREST', () => {
        const postmanScript = `
