...
export async function fetchWrapper(url: URL, method: string, headers: Record<string, string>, body: any, abortControllerSignal: AbortSignal, flags: {
    ...
    try {
        if('__EXTENSION_HOOK__' in window && window.__EXTENSION_HOOK__ === 'PulseREST CORS Helper Enabled') {
...
export async function handleRequest(
    ...
    try {
        ...
        if (!headers['user-agent']) {
            headers['user-agent'] = globalUserAgent || `PulseREST/${getVersion()}`
        }
...
export function convertInsomniaExportToPulseRESTCollection(json: any, workspaceId: string) {
...
function importPulseRESTV1(collections: CollectionItem[], workspaceId: string) {
    const collection: CollectionItem[] = []
...
export function convertPulseRESTExportToPulseRESTCollection(json: any, workspaceId: string) {
    if('exportedFrom' in json) {
        if(json.exportedFrom === 'PulseREST-1.0.0') {
            return importPulseRESTV1(json.collection, workspaceId)
        }
    }

    throw new Error('Invalid PulseREST Export')
}
...
export async function convertOpenAPIExportToPulseRESTCollection(exportString: string, workspaceId: string) {
    ...
    return convertInsomniaExportToPulseRESTCollection(initExport.data, workspaceId)
}

export async function convertCurlCommandToPulseRESTCollection(curlCommand: string, workspaceId: string) {
    ...
    return convertInsomniaExportToPulseRESTCollection({ resources: insomniaExport }, workspaceId)
}
...
export function exportPulseRESTCollection(collection: CollectionItem[], environments = undefined) {
    downloadObjectAsJSON(`PulseREST_${todayISODate()}.json`, {
        exportedFrom: 'PulseREST-1.0.0',
        collection,
        environments,
    })
}
 ...
 */
export function scriptConversion(scriptToConvert: string, scriptType: 'postmanToPulseREST' | 'pulserestToPostman' | 'pulserestToInsomnia') {
    const mappings = {
        postmanToPulseREST: {
    ...
    // Generalize status code conversion for Postman to PulseREST
    if (scriptType === 'postmanToPulseREST') {
...
export async function convertCollectionsFromPulseRESTToPostman(pulserestCollections: any) {
    const pulserestData: any = pulserestCollections
    ...
    pulserestData.forEach((item: any) => {
        if (item._type === 'request_group') {
                    ...
                    if (plugin.enabled && plugin.type === 'script') {
                        if (plugin.code.pre_request) {
                            postmanRequest.event.push({
                                ...
                                script: {
                                    ...
                                    exec: scriptConversion(plugin.code.pre_request, 'pulserestToPostman').trim().split('\n')
                                }
                            })
                        }
                        if (plugin.code.post_request) {
                            postmanRequest.event.push({
                                ...
                                script: {
                                    type: 'text/javascript',
                                    exec: scriptConversion(plugin.code.post_request, 'pulserestToPostman').trim().split('\n')
                                }
                            })
                        }
                    }
...
export async function convertCollectionsFromPulseRESTToInsomnia(pulserestCollections: any) {
    const insomniaCollection: any = {
        _type: 'export',
        __export_format: 4,
        __export_date: new Date().toISOString(),
        __export_source: 'pulserest-to-insomnia-converter',
        resources: []
    }

    const workspaceId = pulserestCollections[0].workspaceId || 'root_workspace'

    const workspace = {
        _id: workspaceId,
        _type: 'workspace',
        name: 'Imported from PulseREST',
        description: '',
        scope: 'collection',
    }
    ...
    // First, create all folders and store them in folderMap
    pulserestCollections.forEach((item: any) => {
        if (item._type === 'request_group') {
    ...
    // Then, create all requests and push them to the appropriate folders in folderMap
    pulserestCollections.forEach((pulserestRequest: any) => {
        if (pulserestRequest._type !== 'request') {
            return
        }

        let body = {}

        if (pulserestRequest.body.mimeType !== 'No Body') {
            body = pulserestRequest.body
        }

        const insomniaRequest: any = {
            _id: pulserestRequest._id,
            _type: 'request',
            parentId: pulserestRequest.parentId || workspaceId,
            name: pulserestRequest.name || pulserestRequest.url,
            method: pulserestRequest.method,
            url: pulserestRequest.url,
            body,
            headers: pulserestRequest.headers,
            authentication: convertPulseRESTAuthToInsomniaAuth(pulserestRequest.authentication),
            parameters: pulserestRequest.parameters,
        }

        const scripts = pulserestRequest.plugins?.find((plugin: any) => plugin.type === 'script')
        if (scripts) {
            insomniaRequest.hook = {
                preRequest: scriptConversion(scripts.code.pre_request, 'pulserestToInsomnia').trim() || '',
                postRequest: scriptConversion(scripts.code.post_request, 'pulserestToInsomnia').trim() || ''
            }
        }
...
function convertPulseRESTAuthToInsomniaAuth(auth: any) {
...
export function convertPostmanAuthToPulseRESTAuth(request: any) {
