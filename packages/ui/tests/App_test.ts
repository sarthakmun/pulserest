...
After(() => {
    I.usePlaywrightTo('clean up DB', async({ page }) => {
        await page.evaluate(() => {
            window.indexedDB.deleteDatabase('PulseREST')
        })
    })
})

Scenario('Check page title', () => {
    I.seeTitleEquals('PulseREST')
})
...
Scenario('Send GET request', async() => {
    const host = 'https://httpbingo.org'
    const path = '/get'
    ...
    I.waitForElement('//*[@class="response-panel-tab"][text() = "Timeline "]', 10)
