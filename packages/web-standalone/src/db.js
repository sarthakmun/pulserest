...
// idMap is scoped per workspace location to avoid cross-workspace collisions
// when multiple workspaces or browser tabs are active simultaneously.
// Key: workspace.location (absolute path), Value: Map<collectionId, absolutePath>
const workspaceIdMaps = new Map()
const fsLog = []

function getIdMap(workspaceLocation) {
    if (!workspaceIdMaps.has(workspaceLocation)) {
        workspaceIdMaps.set(workspaceLocation, new Map())
    }
    return workspaceIdMaps.get(workspaceLocation)
}
...
async function getWorkspaceAtLocation(location, getEnvironments = false) {
    logMethodCall('getWorkspaceAtLocation', { location, getEnvironments })
...
async function updateWorkspace(workspace, updatedFields) {
    logMethodCall('updateWorkspace', { workspace, updatedFields })
    ...
    if (updatedFields.currentEnvironment) { update.currentEnvironment = updatedFields.currentEnvironment }
    if (Object.keys(update).length === 0) { console.log('No fields to update for workspace'); return }
    const workspaceUpdated = { ...workspaceExisting, ...update }
...
async function ensureEmptyFolderOrEmptyWorkspace(location) {
    ...
    if (!pathExists) { return { error: `Given folder path does not exist: ${location}` } }
    ...
    if (workspaceConfigExists) {
        if (ls.length === 1 && ls[0] === constants.FILES.WORKSPACE_CONFIG) { return { error: null } }
        else { return { error: `Given folder path is not an empty PulseREST workspace: ${location}` } }
    }
    if (ls.length === 0) { return { error: null } }
    else { return { error: `Given folder path is not empty: ${location}` } }
}

async function getCollectionForWorkspace(workspace, type) {
    logMethodCall('getCollectionForWorkspace', { workspace, type })

    try {
        let idMapBeforeClear = null
        const idMap = getIdMap(workspace.location)
        ...
        if (type === null) {
            ...
            await dbHelpers.ensurePulseRESTCollection(workspace)
        }
        ...
        if (type === 'request_group') {
            return { error: null, collection: collection.filter((item) => item._type === 'request_group'), workspace: null, idMap: null }
        }

        return { error: null, collection, workspace: workspaceData, idMap: idMapBeforeClear ? [...idMapBeforeClear.entries()] : null }
    } catch (error) {
        return { error: { message: error.message }, collection: [], workspace: null, idMap: null }
    }
}

function getCollectionById(workspace, collectionId) {
    logMethodCall('getCollectionById', { workspace, collectionId })
    const idMap = getIdMap(workspace.location)
...
async function createCollection(workspace, collection) {
    logMethodCall('createCollection', { workspace, collection })
    const idMap = getIdMap(workspace.location)
    ...
    try {
        ...
        if (collection._type === 'request_group') {
            ...
            if (collection.sortOrder !== undefined) { collectionToSave.sortOrder = collection.sortOrder }
        ...
        return { error: null, oldCollectionId, newCollectionId: collectionPathWithoutWorkspaceLocation }
    } catch (err) {
        if (err.code === 'EEXIST') { return { error: `Cannot create ${collectionName} as it already exists` } }
        console.log(err)
        return { error: `Error creating collection: ${collectionName}: ${err.message}` }
    }
}

async function createCollections(workspace, collections) {
    logMethodCall('createCollections', { workspace, collections })
    const idMap = getIdMap(workspace.location)
    const results = []
    for (const collection of collections) {
        let result = null
        if (idMap.get(collection._id)) {
            result = await updateCollection(workspace, collection._id, { sortOrder: collection.sortOrder })
            if (result === undefined) { result = { error: null, oldCollectionId: null, newCollectionId: null } }
        } else {
        ...
        if (result && result.error) { return { error: result.error, results } }
        results.push(result)
    }
    return { error: null, results }
}

async function updateIdMapForChildren(oldBasePath, newBasePath, workspaceLocation, ignoreId = null) {
    const idMap = getIdMap(workspaceLocation)
    ...
    for (const item of items) {
        ...
        if (item.isDirectory()) {
            await updateIdMapForChildren(oldItemPath, newItemPath, workspaceLocation, ignoreId)
        }
    }
}

async function updateCollection(workspace, collectionId, updatedFields) {
    logMethodCall('updateCollection', { workspace, collectionId, updatedFields })
    const idMap = getIdMap(workspace.location)
    const collectionPath = idMap.get(collectionId)
    ...
    if (Object.keys(updatedFields).length === 1 && 'name' in updatedFields) {
        ...
        if (renameFrom.endsWith('.json')) {
            ...
            try { await fs.access(responsesRenameFrom); await fileUtils.renameFileOrFolder(responsesRenameFrom, responsesRenameTo, fsLog, `Rename responses file`) } catch (err) { console.log(`Skipping renaming responses file: ${responsesRenameFrom} as it does not exist`) }
            ...
            try { await fs.access(pluginsRenameFrom); await fileUtils.renameFileOrFolder(pluginsRenameFrom, pluginsRenameTo, fsLog, `Rename plugins file`) } catch (err) { console.log(`Skipping renaming plugins file: ${pluginsRenameFrom} as it does not exist`) }
            ...
            try { await fs.access(messagesRenameFrom); await fileUtils.renameFileOrFolder(messagesRenameFrom, messagesRenameTo, fsLog, `Rename messages file`) } catch (err) { console.log(`Skipping renaming messages file: ${messagesRenameFrom} as it does not exist`) }
        } else {
            await updateIdMapForChildren(renameFrom, renameTo, workspace.location, collectionId)
        }
    ...
    if (Object.keys(updatedFields).length === 1 && 'parentId' in updatedFields) {
        ...
        try {
            ...
            return { error: `Error renaming ${renameFrom} to ${renameTo}: ${err.message}` }
        }
        if (renameFrom.endsWith('.json')) {
            const responsesRenameFrom = renameFrom.replace('.json', constants.FILES.RESPONSES)
            const responsesRenameTo = renameTo.replace('.json', constants.FILES.RESPONSES)
            ...
            await updateIdMapForChildren(renameFrom, renameTo, workspace.location)
        }
    ...
    if (Object.keys(updatedFields).length === 1 && ('sortOrder' in updatedFields || 'currentEnvironment' in updatedFields || 'environments' in updatedFields || 'collapsed' in updatedFields || 'headers' in updatedFields || 'authentication' in updatedFields)) {
        ...
        if('collapsed' in updatedFields) {
            ...
            if (updatedFields.collapsed) {
                try { await fileUtils.writeEmptyFileNewOnly(collapsedFilePath, fsLog, `Mark directory as collapsed`); console.log(`Marked as collapsed: ${collapsedFilePath}`) } catch (err) { console.error(`Error marking directory as collapsed: ${collapsedFilePath}`, err) }
            } else {
                try { await fileUtils.deleteFileOrFolder(collapsedFilePath, fsLog, `Mark directory as expanded`); console.log(`Marked as expanded (collapsed file removed): ${collapsedFilePath}`) } catch (err) { console.error(`Error marking directory as expanded: ${collapsedFilePath}`, err) }
            }
            return { error: null }
        }
        ...
        if (stats.isDirectory()) { collectionPathCopy = path.join(collectionPath, constants.FILES.FOLDER_CONFIG) }
        let collectionExisting = {}
        try { collectionExisting = JSON.parse(await fs.readFile(collectionPathCopy, 'utf8')) } catch {}
        const collectionUpdated = { ...collectionExisting, ...updatedFields }
    ...
    if (updatedFields._type === 'request' || updatedFields._type === 'socket') {
        ...
        if (updatedFields._type === 'socket') {
            if (updatedFields.clients) {
                ...
                try { existingMessages = JSON.parse(await fs.readFile(collectionPath.replace('.json', constants.FILES.MESSAGES), 'utf8')) } catch {}
                const clientIds = []
                ...
                Object.keys(existingMessages).forEach((clientId) => { if(!clientIds.includes(clientId)) { delete existingMessages[clientId] } })
...
async function deleteCollectionsByWorkspaceId(workspace) {
    logMethodCall('deleteCollectionsByWorkspaceId', { workspace })
    const idMap = getIdMap(workspace.location)
    const items = await fs.readdir(workspace.location, { withFileTypes: true })
    for (const item of items) {
        const itemPath = path.join(workspace.location, item.name)
        if (item.name.endsWith('.git')) { continue }
        if (item.isDirectory()) {
            const folderConfigExists = await fileUtils.pathExists(path.join(itemPath, constants.FILES.FOLDER_CONFIG))
            if (folderConfigExists || item.name === constants.FOLDERS.ENVIRONMENTS) { await fs.rm(itemPath, { recursive: true }) }
        } else {
            if (item.name.endsWith('.json')) { await fileUtils.deleteFileOrFolder(itemPath, fsLog, `Delete collection item`) }
        }
    }
    for (const [id, pathValue] of idMap) {
        if (pathValue.startsWith(workspace.location)) { idMap.delete(id) }
    }
    console.log(`Cleared directory: ${workspace.location}`)
}

async function deleteCollectionsByIds(workspace, collectionIds) {
    logMethodCall('deleteCollectionsByIds', { workspace, collectionIds })
    const idMap = getIdMap(workspace.location)
    try {
        ...
        for (const collectionItem of collectionItemsToDelete) {
            ...
            if (collectionItem.collectionPath.endsWith('.json') === false) {
                ...
                try { await fileUtils.deleteFileOrFolder(path.join(collectionItem.collectionPath, constants.FILES.COLLAPSED), fsLog, `Delete collection item collapsed state marker`) } catch { console.log(`No collapsed state marker found for deletion for collection item: ${collectionItem.collectionPath}`) }
                try {
                    const environmentsDir = await fs.readdir(path.join(collectionItem.collectionPath, constants.FOLDERS.ENVIRONMENTS))
                    for (const envFile of environmentsDir) { await fileUtils.deleteFileOrFolder(path.join(collectionItem.collectionPath, constants.FOLDERS.ENVIRONMENTS, envFile), fsLog, `Delete collection item environment`) }
                ...
                } catch { console.log(`No environments found for deletion for collection item: ${collectionItem.collectionPath}`) }
            } else {
                try { await fileUtils.deleteFileOrFolder(collectionItem.collectionPath.replace('.json', constants.FILES.MESSAGES), fsLog, `Delete messages for collection item`) } catch { console.log(`No messages found for deletion for collection item: ${collectionItem.collectionPath}`) }
            }
        ...
        console.error('deleteCollectionsByIds', err)
        return { error: `Error deleting collections: ${err.message}` }
    }
}

async function getResponsesByCollectionId(workspace, collectionId) {
    logMethodCall('getResponsesByCollectionId', { workspace, collectionId })
    const idMap = getIdMap(workspace.location)
    const collectionPath = idMap.get(collectionId)
    if (collectionPath.endsWith('.json') === false) { return [] }
    ...
    if (responsesPathExists) {
        ...
        responses.forEach((response) => { dbHelpers.deserializeRequestResponseFiles(response); response.collectionId = collectionId })
...
async function createResponse(workspace, response) {
    logMethodCall('createResponse', { workspace, response })
    const idMap = getIdMap(workspace.location)
    const collectionPath = idMap.get(response.collectionId)
    const responsesPath = collectionPath.replace('.json', constants.FILES.RESPONSES)
    const responsesPathExists = await fileUtils.pathExists(responsesPath)
    let responses = []
    if (responsesPathExists) { responses = JSON.parse(await fs.readFile(responsesPath, 'utf8')) }
...
async function updateResponse(workspace, collectionId, responseId, updatedFields) {
    logMethodCall('updateResponse', { workspace, collectionId, responseId, updatedFields })
    const idMap = getIdMap(workspace.location)
    const collectionPath = idMap.get(collectionId)
    const responsesPath = collectionPath.replace('.json', constants.FILES.RESPONSES)
    const responses = JSON.parse(await fs.readFile(responsesPath, 'utf8'))
    ...
    responses[responseIndex] = { ...responses[responseIndex], ...updatedFields }
...
async function deleteResponse(workspace, collectionId, responseId) {
    logMethodCall('deleteResponse', { workspace, collectionId, responseId })
    const idMap = getIdMap(workspace.location)
    const collectionPath = idMap.get(collectionId)
    const responsesPath = collectionPath.replace('.json', constants.FILES.RESPONSES)
    const responses = JSON.parse(await fs.readFile(responsesPath, 'utf8'))
    ...
    if (responses.length === 0) { await fileUtils.deleteFileOrFolder(responsesPath, fsLog, `Delete responses file`); console.log(`Deleted responses file: ${responsesPath}`); return }
...
async function deleteResponsesByIds(workspace, collectionId, responseIds) {
    logMethodCall('deleteResponsesByIds', { workspace, collectionId, responseIds })
    const idMap = getIdMap(workspace.location)
    const collectionPath = idMap.get(collectionId)
    const responsesPath = collectionPath.replace('.json', constants.FILES.RESPONSES)
...
async function deleteResponsesByCollectionIds(workspace, collectionIds) {
    logMethodCall('deleteResponsesByCollectionIds', { workspace, collectionIds })
    const idMap = getIdMap(workspace.location)
    for (const collectionId of collectionIds) {
        const collectionPath = idMap.get(collectionId)
        if (!collectionPath) { console.log(`Skipping deleting responses for collection: ${collectionId} as it does not exist`); continue }
        const stats = await fs.stat(collectionPath)
        if (stats.isDirectory()) { console.log(`Skipping deleting responses for directory: ${collectionPath}`); continue }
        const responsesPath = collectionPath.replace('.json', constants.FILES.RESPONSES)
        try { await fs.access(responsesPath) } catch (err) { console.log(`Skipping deleting responses: ${responsesPath} as it does not exist`); continue }
...
async function deleteResponsesByCollectionId(workspace, collectionId) {
    logMethodCall('deleteResponsesByCollectionId', { workspace, collectionId })
    const idMap = getIdMap(workspace.location)
    const collectionPath = idMap.get(collectionId)
    const responsesPath = collectionPath.replace('.json', constants.FILES.RESPONSES)
    await fileUtils.deleteFileOrFolder(responsesPath, fsLog, `Delete responses for collection item`)
...
async function getWorkspacePlugins(workspace) {
    logMethodCall('getWorkspacePlugins', { workspace })
    const idMap = getIdMap(workspace.location)
    const items = []
    ...
    try {
        ...
        workspacePluginsData.forEach((plugin) => { plugin.workspaceId = workspace._id; plugin.collectionId = null })
    ...
    for (const item of collectionItems) {
        ...
        if (item._type === 'request_group') {
            ...
            try { const collectionPluginsData = JSON.parse(await fs.readFile(collectionPluginsPath, 'utf8')); collectionPluginsData.forEach((plugin) => { plugin.collectionId = item._id }); items.push(...collectionPluginsData) } catch (err) {}
        } else {
            const itemPluginsPath = collectionPath.replace('.json', constants.FILES.PLUGINS)
            try { const itemPluginsData = JSON.parse(await fs.readFile(itemPluginsPath, 'utf8')); itemPluginsData.forEach((plugin) => { plugin.collectionId = item._id }); items.push(...itemPluginsData) } catch (err) {}
        }
    }
    return items
}
...
async function createPlugin(workspace, plugin) {
    logMethodCall('createPlugin', { workspace, plugin })
    const idMap = getIdMap(workspace.location)
    let pluginPath = ''
    if (plugin.collectionId) {
        const collectionPath = idMap.get(plugin.collectionId)
        const stats = await fs.stat(collectionPath)
        if (stats.isDirectory()) { pluginPath = path.join(collectionPath, constants.FILES.FOLDER_PLUGINS) }
        else { const collectionItemName = path.basename(collectionPath, '.json'); const collectionPathParentPath = path.dirname(collectionPath); pluginPath = `${path.join(collectionPathParentPath, collectionItemName)}${constants.FILES.PLUGINS}` }
    } else { pluginPath = path.join(workspace.location, constants.FILES.WORKSPACE_PLUGINS) }
    try {
        let existingPlugins = []
        try { existingPlugins = JSON.parse(await fs.readFile(pluginPath, 'utf8')) } catch (err) {}
    ...
    } catch (err) { console.error(`Error creating plugin in: ${pluginPath}`, err) }
}

async function updatePlugin(workspace, collectionId, pluginId, updatedFields) {
    logMethodCall('updatePlugin', { workspace, collectionId, pluginId, updatedFields })
    const idMap = getIdMap(workspace.location)
    const collectionPath = idMap.get(collectionId)
    let pluginPath = null
    if (collectionId == null) { pluginPath = path.join(workspace.location, constants.FILES.WORKSPACE_PLUGINS) }
    else { pluginPath = collectionPath.endsWith('.json') ? collectionPath.replace('.json', constants.FILES.PLUGINS) : path.join(collectionPath, constants.FILES.FOLDER_PLUGINS) }
    try {
        ...
        } else { console.log(`Plugin ${pluginId} not found for update in ${pluginPath}`) }
    } catch (err) { console.error(`Error updating plugin ${pluginId} in ${pluginPath}`, err) }
}

async function deletePlugin(workspace, collectionId, pluginId) {
    logMethodCall('deletePlugin', { workspace, collectionId, pluginId })
    const idMap = getIdMap(workspace.location)
    if (collectionId === null) {
        ...
        try {
            ...
            if (filteredPlugins.length < existingPlugins.length) {
                if (filteredPlugins.length === 0) { await fileUtils.deleteFileOrFolder(workspacePluginsPath, fsLog, `Delete workspace plugins`); console.log(`All workspace plugins deleted: ${workspace.location}`) }
                else { await fileUtils.writeFileJson(workspacePluginsPath, filteredPlugins, fsLog, `Delete plugin`); console.log(`Plugin ${pluginId} deleted in ${workspacePluginsPath}`) }
            } else { console.log(`Plugin ${pluginId} not found for deletion in ${workspacePluginsPath}`) }
        } catch (err) { console.error(`Error deleting plugin ${pluginId} in ${workspacePluginsPath}`, err) }
        return
    }
    const collectionPath = idMap.get(collectionId)
    ...
    try {
        ...
        } else { console.log(`Plugin ${pluginId} not found for deletion in ${pluginPath}`) }
    } catch (err) { console.error(`Error deleting plugin ${pluginId} in ${pluginPath}`, err) }
}

async function deletePluginsByWorkspace(workspace) {
    logMethodCall('deletePluginsByWorkspace', { workspace })
    const workspacePluginsPath = path.join(workspace.location, constants.FILES.WORKSPACE_PLUGINS)
    try { await fs.access(workspacePluginsPath); await fileUtils.deleteFileOrFolder(workspacePluginsPath, fsLog, `Delete workspace plugins`); console.log(`All workspace plugins deleted: ${workspace.location}`) }
    catch (err) { console.log(`No workspace plugins found for deletion in: ${workspace.location}`) }
}

async function deletePluginsByCollectionIds(workspace, collectionIds) {
    logMethodCall('deletePluginsByCollectionIds', { workspace, collectionIds })
    const idMap = getIdMap(workspace.location)
    for (const collectionId of collectionIds) {
        const collectionPath = idMap.get(collectionId)
        if (!collectionPath) { console.log(`Skipping deleting plugins for collection: ${collectionId} as it does not exist`); continue }
        const pluginPath = collectionPath.endsWith('.json') ? collectionPath.replace('.json', constants.FILES.PLUGINS) : path.join(collectionPath, constants.FILES.FOLDER_PLUGINS)
        try { await fileUtils.deleteFileOrFolder(pluginPath, fsLog, `Delete plugins for collection item`); console.log(`Deleted plugins for collection: ${collectionId}`) }
        catch(err) { console.log(`No plugins found for deletion for collection: ${collectionId}`) }
    }
}

async function createPlugins(workspace, plugins) {
    logMethodCall('createPlugins', { workspace, plugins })
    for (const plugin of plugins) { await createPlugin(workspace, plugin) }
}
